...
from gif_search import resolve_gif_queries, reset_used_gifs, register_used_url
...
def _seed_used_gifs_from_semaine() -> None:
    """Charge les image_url déjà utilisées dans commentaires_semaine.json
    pour éviter de les réutiliser."""
    ...
    try:
        ...
        return
    for jour_entries in data.values():
        for p in jour_entries or []:
            for c in p.get("commentaires", []) or []:
                register_used_url(c.get("image_url", ""))
...
def _fetch_profiles_from_api() -> list[dict] | None:
    """Tente de charger les profils depuis l'API Vercel (source de vérité).
    Retourne None si indisponible."""
    import os
    api_url = os.environ.get("VERCEL_API_URL")
    if not api_url:
        return None
    try:
        import requests
        base = api_url.rstrip("/").replace("/api/update", "")
        r = requests.get(f"{base}/api/ia", timeout=10)
        ...
        print(f"[comment_agent] Fetch API profils échoué: {e}", flush=True)
        return None


def _load_personnages() -> list[dict]:
    """Charge les personnages : priorité à l'API Vercel (éditable depuis le site),
    fallback sur les fichiers JSON locaux. Filtre les inactifs."""
    remote = _fetch_profiles_from_api()
    if remote is not None:
        print(f"[comment_agent] {len(remote)} profils chargés depuis l'API.", flush=True)
        personnages = []
        for p in remote:
            if p.get("actif") is False:
                print(f"[comment_agent]   → {p.get('prenom')} désactivé, ignoré.", flush=True)
                continue
...
def _build_personnages_prompt(personnages: list[dict]) -> str:
    ...
    for p in personnages:
        ...
        gifs_fetiches = p.get("gifs_fetiches") or []
        if gifs_fetiches:
            lines.append(
                f"  GIFs fétiches (URLs à réutiliser parfois via le champ 'image_url' directement, au lieu de 'gif_query') :"
            )
            for url in gifs_fetiches[:5]:
                lines.append(f"    - {url}")
...
def generate_commentaires_jour(plats: list[dict]) -> list[dict]:
    ...
    # Éviter de réutiliser les GIFs déjà vus cette semaine.
    _seed_used_gifs_from_semaine()
...
def generate_commentaires_semaine(
    ...
    # Nouvelle semaine : on repart d'une mémoire vierge de GIFs.
    reset_used_gifs()
...
def generate_commentaires_personnage(prenom: str) -> dict[str, list[dict]]:
    ...
    # Éviter de repicker un GIF déjà utilisé par les autres personnages.
    _seed_used_gifs_from_semaine()
